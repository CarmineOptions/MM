...
class POCMMModel:
    ...
    def get_optimal_orders(self, account: WAccount, state_market) -> List[Order]:
        return self._get_optimal_quotes(
            ...
            fair_price=state_market.oracle['price']
        )


    def _get_optimal_quotes(self, asks, bids, market_maker_cfg, market_cfg, fair_price):
        """
        ...
        for side, side_name in [(asks, 'ask'), (bids, 'bid')]:
            ...
            for order in side:
                ...
                if (
                    ...
                    self._logger.info(f"Canceling order because too close to FP. fair_price: {fair_price}, order price: {order['price'] / 10**base_decimals}")
