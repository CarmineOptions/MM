...
class RemusMarketConfig(MarketConfig):
    ...
    def from_dict(cfg: dict[str, Any], market_id: int) -> "RemusMarketConfig | None":
        ...
        if base_token is None or quote_token is None:
            logging.error(
                f"Unable to find tokens for base/quote: {cfg['base_token']}/{cfg['quote_token']}, market_id: {market_id}"
            )
            return None
...
ETH_USDC_MC = RemusMarketConfig(
    ...
    fees=RemusFeesConfig(taker_fee_bps=0, maker_fee_bps=0),
)
